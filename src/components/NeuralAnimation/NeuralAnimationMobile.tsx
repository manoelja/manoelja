...
const NeuralAnimationMobile = ({ className, style, isGlobal }: NeuralAnimationProps) => {
  ...
  useEffect(() => {
    ...
    const particleCount = isGlobal ? 80 : 50;
    const connectionDistance = isGlobal ? 120 : 80;
    const mouseRadius = 150;
    const mouseConnectionDist = isGlobal ? 120 : 0;
    ...
    class Particle {
      ...
      constructor(x: number, y: number, isTrail = false) {
        ...
        this.vx = (Math.random() - 0.5) * (isGlobal ? 0.15 : 0.35);
        this.vy = (Math.random() - 0.5) * (isGlobal ? 0.15 : 0.35);
        this.baseSize = Math.random() * (isGlobal ? 0.9 : 1.2) + 0.5;
        this.size = this.baseSize;
        this.opacity = isTrail ? 0.4 : (Math.random() * 0.3 + 0.2);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isGlobal && Math.random() > 0.97) {
        trailParticles.push(new Particle(newX, newY, true));
        if (trailParticles.length > 12) trailParticles.shift();
      }
    };
      ...
      for (let i = 0; i < allParticles.length; i++) {
        ...
        for (let j = i + 1; j < allParticles.length; j++) {
          ...
          if (distance < connectionDistance) {
            ...
            let alpha = (1 - distance / connectionDistance) * (isGlobal ? 0.6 : 0.7);
  ...
  return (
    <motion.div 
      ...
      style={{ ...style, y: isGlobal ? parallaxY : 0, opacity: isGlobal ? 0.4 : 1 }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
export default NeuralAnimationMobile;
